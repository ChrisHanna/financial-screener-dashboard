...
function addAdvancedPatternsToChart(chart, data) {
    ...
    if (data.divergences) {
        ...
        if (data.divergences.mfBullish && data.divergences.mfBullish.length > 0) {
            chart.data.datasets.push({
                label: 'MF Bullish Divergence',
                data: data.divergences.mfBullish.map(date => ({
                    x: new Date(date),
                    y: -85
                })),
        ...
        if (data.divergences.mfBearish && data.divergences.mfBearish.length > 0) {
            chart.data.datasets.push({
                label: 'MF Bearish Divergence',
                data: data.divergences.mfBearish.map(date => ({
                    x: new Date(date),
                    y: 85
                })),
    ...
    if (data.patterns) {
        ...
        if (data.patterns.rsiTrendBreakBuy && data.patterns.rsiTrendBreakBuy.length > 0) {
            chart.data.datasets.push({
                ...
                data: data.patterns.rsiTrendBreakBuy.map(date => ({
                    x: new Date(date),
                    y: -70
                })),
        ...
        if (data.patterns.rsiTrendBreakSell && data.patterns.rsiTrendBreakSell.length > 0) {
            chart.data.datasets.push({
                ...
                data: data.patterns.rsiTrendBreakSell.map(date => ({
                    x: new Date(date),
                    y: 70
                })),
    ...
    displaySignals.forEach(signal => {
        ...
        if (signal.type === 'Buy') {
            signalClass = signal.subType === 'Bullish Divergence' ? 'text-success fw-bold' :
                          signal.subType === 'Fast Money' ? 'text-info' : 'text-success';
        } else { // Sell
            signalClass = signal.subType === 'Bearish Divergence' ? 'text-danger fw-bold' :
                          signal.subType === 'Fast Money' ? 'text-pink' : 'text-danger';
        }
...
function renderTrendExhaustChart(detailedData) {
    ...
    // ... existing code for local calculation
}
